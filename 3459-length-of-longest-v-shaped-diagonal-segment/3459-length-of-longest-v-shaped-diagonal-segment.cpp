...
public:
    ...
    int dfs(int r, int c, vector<vector<int>>& grid, int exp, int n, int m,
        ...
        int nexp = exp == 2 ? 0 : 2;
        int maxLen1 = 1 + dfs(nr, nc, grid, nexp, n, m, d, dir, canTurn, dp);
        int maxLen2 = -1e9;
        if (canTurn) {
            maxLen2 =
                1 + dfs(nr, nc, grid, nexp, n, m, (d + 1) % 4, dir, 0, dp);
        }
        return dp[r][c][d][canTurn] = max(maxLen1, maxLen2);
    }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (grid[i][j] == 1) {
                    for (int d = 0; d < 4; d++) {
                        ans = max(ans,
                                  1 + dfs(i, j, grid, 2, n, m, d, dir, 1, dp));
                    }
                }
            }
        }
